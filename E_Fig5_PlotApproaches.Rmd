...
MedianValues %>%
  ...
  geom_text(data=Labels, aes(x,y, label =Text), size = 3)-> SubPlotA_JustDot
SubPlotA_JustDot
```
...
MedianValues %>%
  ...
  geom_path(arrow = arrow(length = unit(0.5, "cm")), size =2)+
  theme_classic()+
  scale_colour_manual(values = c('darkred','blue')) -> SubPlotB_Errors
SubPlotB_Errors
...
Both_TreatmentsValues %>%
  ...
  scale_colour_manual(values = c('darkred','blue'))-> SubPlotD_FullPost
SubPlotD_FullPost
```
...
d%>%
  ...
  geom_scatterpie(aes(x = MEDND, y = MEDFD, group = Treatment), data = d,
                  pie_scale = 50,
                  cols= c('Species 1 Wins', 'Species 2 Wins','Coexistence' ,'Priority'))+
  scale_y_log10(name = expression(paste("Fitness difference: ",kappa[j]/kappa[i])) )+
  scale_fill_manual(name = 'Outcome',
                    values =c('yellow', 'red', 'darkgreen', 'purple'))+
  xlab(expression(paste("Niche Difference: ", 1-rho)))+
  coord_fixed(xlim =c(-0.25, 1), ylim= c(0.5, 6.5))+
...
plot_grid(Top,Mid, Low,ncol=1)

ggsave('Figures/Fig5_PlotOptions.png', width=10, height = 12, dpi = 600, bg = 'white')
