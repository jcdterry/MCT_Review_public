...
Mean_Epred_allMods %>%
  ...
  coord_cartesian(ylim = c(-1.5, 8), xlim = c(0, 45))+
  scale_y_continuous(breaks = c(0,1:8)) +
  ylab('Species A Reproductive Rate')+
  xlab('Number of Intraspecific (Species A) Competitors')+
  ...
  ggtitle('a) Species A monoculture equilibrium fit') -> SpeciesAEqPlot_WO
...
Mean_Epred_allMods_BINVASION %>%
  ...
  xlab('Number of Interspecific (Species A) Competitors')+
  ...
  ggtitle('b) Species B invasion growth rate') -> SpeciesB_InvPlot_WO

model_plots_WO = plot_grid(SpeciesAEqPlot_WO,
                           SpeciesB_InvPlot_WO+guides(fill=FALSE,col=FALSE),
                           ncol = 2, rel_heights = c(3,3))

model_plots_WO

ggsave(plot = model_plots_WO,'Figures/Figure3ab.pdf', width = 8, height = 4)
ggsave(plot = model_plots_WO,'Figures/Figure3ab.png', width = 8, height = 4,
       dpi= 600, bg = 'white')

```

# SI version, with model plots
...
Mean_Epred_allMods %>%
  ...
  theme(legend.justification = 'center',
        ...
        legend.title = element_text(size = 16),  
        legend.text = element_text(size = 16), 
        legend.background = element_rect(colour = 'white')) -> SpeciesAEqPlot
...
Mean_Epred_allMods_BINVASION %>%
  ...
  geom_pointrange(data = segdat_inv,
                  aes(y = yval_inv, x = xpos_inv, ymin = CIlo_inv,
                      ymax = CIhi_inv, color = Model), inherit.aes = F) -> SpeciesB_InvPlot

model_plots = plot_grid(SpeciesAEqPlot+
                          coord_cartesian(ylim = c(-1.5, 8), xlim = c(0, 45))+
                          scale_y_continuous(breaks = c(0,1:8)) +
  ggtitle('a) Species A monoculture equilibrium fit'),
                        SpeciesB_InvPlot+
                          coord_cartesian(ylim = c(0, 5), xlim = c(0, 40))+
                          scale_x_continuous(breaks = c(0,10, 20, 30))+
                          guides(fill=FALSE,col=FALSE)+
  ggtitle('b) Species B invasion growth rate'),
                        ncol = 2, rel_heights = c(3,3))
```
...
```{R}
Facetted_fits<- plot_grid(SpeciesAEqPlot+
            facet_grid(~Model)+
            theme(strip.text = element_blank())+
  ggtitle('c) Species A monoculture equilibrium fit'),
          SpeciesB_InvPlot+
            facet_grid(~Model)+
            theme(strip.text = element_blank())+
                          guides(fill=FALSE,col=FALSE)+
  ggtitle('d) Species B invasion growth rate'), nrow=2)
```
...
plot_grid(model_plots,Facetted_fits, ncol = 1)


ggsave('Figures/FigureSI2.pdf', width = 10, height = 12)
ggsave('Figures/FigureSI2.png', width = 10, height = 12,
       dpi= 600, bg = 'white')
```
